...
%         NRF=1;
        supp_ctr(ii)=0;
        del_p_comp=-1.00*del_pv; %Q_CloudyPeriod;%
        P_ref(ii)=(P_ref_mem(ii)+del_p_comp);%Qs;%
    elseif del_pv>(5)
%         disp('2')
%         PRF=1;
        supp_ctr(ii)=0;
        ...
        if (P_ref_mem(ii)>0.00)&&(supp_ctr(ii)>=(3*tsc))
%             disp('3')
            del_p_comp=(1);
            P_ref(ii)=(P_ref_mem(ii)-del_p_comp);
        elseif (P_ref_mem(ii)>0.00)&&(supp_ctr(ii)<(3*tsc))
%             disp('4')
            supp_ctr(ii)=supp_ctr(ii)+1;
            del_p_comp=1;
            P_ref(ii)=(P_ref_mem(ii)-1*del_p_comp);
        elseif (P_ref_mem(ii)<=0.00);%&&(supp_ctr(ii)>=5*60)
            if PR_pcc(ii)<=0
%                 disp('5')
                del_p_comp=0;
                P_ref(ii)=0*(P_ref_mem(ii)+del_p_comp);
            elseif PR_pcc(ii)>0
%                 disp('6')
%                 if abs((abs(P_ref_mem(ii))-abs(P_shape_ref(ii))))>0.01%(((abs(P_ref_mem(ii))-abs(P_shape_ref(ii)))<-0.0001)||(abs(P_ref_mem(ii))-abs(P_shape_ref(ii)))>0.0001)%abs(abs(P_ref_mem(ii))-abs(P_shape_ref(ii)))>abs((abs(P_ref_mem(ii))-abs(P_shape_ref(ii)))/10)%
                    P_ref(ii)=P_ref_mem(ii)+((abs(P_ref_mem(ii))-abs(P_shape_ref(ii)))/5);
%                     P_ref(ii)=P_ref_mem(ii)-0.001;
%                 else
